var warOfLife = (function() {
	...
	return {
		init: function(size, playersMap) {
			battleFieldSize = size;
			battleField = new Array(size);

			for (i = 0; i < size; i++)
				battleField[i] = new Array(size);
...
var gameHandler = (function() {
	var players,
		...
		generationsCount,
		generationLength,
		start = function(size, generations, genLength) {
			mapSize = size;
			generationsCount = generations;
			generationLength = genLength;
			$.getJSON( "http://localhost:8080/players", function( data ) {
            players = data;
    			playRound(prepareRoundPlayersMap());
          });
		},
		playRound = function(map) {
			warOfLife.init(mapSize, map);
